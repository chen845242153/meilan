        ...
        if (isEmpty) {
            ...
            for (int i = 0; i < timeList.length; i++) {
                if (!TextUtils.isEmpty(timeList[i])) {
                    canvas.drawText(timeList[i], AppUtils.dipToPx(getContext(), 38) + i * itemWidth, totalHeight + itemHeight * 0.5f + AppUtils.dipToPx(getContext(), 20), textPaint);
                }
            }
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                int padding = AppUtils.dipToPx(getContext(), 6);
